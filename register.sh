...
elastix_fixed_input_dir="data/preprocessed"
...
for image_file in "${image_list[@]}"; do
  ...
  elastix -m "$elastix_moving_input_dir/${image_file}/${image_file}_iBHCT.nii.gz" -f "$elastix_fixed_input_dir/${image_file}/${image_file}_eBHCT.nii.gz" -out "$elastix_output_subdir" -p "$param_affine" -p "$param_elastic" -fp "$elastix_moving_input_dir/${image_file}/${image_file}_eBH_xyz_r1.txt" -mp "$elastix_moving_input_dir/${image_file}/${image_file}_iBH_xyz_r1.txt" 
